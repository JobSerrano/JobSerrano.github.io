...
formCalc.addEventListener("submit", function (event) {
    ...
    if (calculoActual === "I") {
        ...
        resTexto = `<span class="formula">I</span> = <span class="formula">${formatearResultado(Icalc)} A</span>`;
    } else if (calculoActual === "vd") {
        const vdCalc = I / (n * A * e);
        resTexto = `<span class="formula">v<sub>d</sub></span> = <span class="formula">${formatearResultado(vdCalc)} m/s</span>`;
    } else if (calculoActual === "J") {
        const Jcalc = I / A;
        resTexto = `<span class="formula">J</span> = <span class="formula">${formatearResultado(Jcalc)} A/m²</span>`;
    }

    resultado.innerHTML = `<p> ${resTexto}</p>`;
